...
import {SortingState, IntegratedSorting, IntegratedPaging, DataTypeProvider, PagingState, EditingState,
  ...
  import {Grid, Table, TableColumnResizing, TableHeaderRow, TableEditColumn, DragDropProvider, TableColumnVisibility, PagingPanel,
    Toolbar, SearchPanel, TableRowDetail, TableSelection} from '@devexpress/dx-react-grid-material-ui'
    ...
    class ImgTable extends React.Component {
      ...
      getOrder = async state =>{
        try{
          ...
          let data = {desc: this.state.searchDesc,
            name: this.state.searchName, tag: searchTag
          }
          const response = await axios.post(image_upload_url,
            ...
            if(response.data.total>0){
              ...
              rows.forEach(row=>{
                row._idh = <div><img style={{"marginBottom":"-0.42rem"}}
                src={"http://192.168.0.96:8080/try/pages/sfc2/2.3/scripts/file_handler_icon.php?id="+row._idh}/>
                <Button onClick={this.editHandler.bind(this, row._idh)}>edit {row._idh}</Button>
                <Button onClick={this.deleteHandler.bind(this, row._idh)}>delete</Button>
                </div>
      ...
      async getCustomInfo(id) {
        try {
          return await axios.get('http://192.168.0.96:8080/try/pages/sfc2/2.3/scripts/custom_handler_react.php?id=' + id)
        } catch (error) {
          console.error(error)
        }
      }
      ...
      editHandler = (value, ...props) => {
        ...
        customInfo.then(response => {
          let description = response.data.description
          let tags = []
          for (var i in response.data.rows) {
            tags.push({name: response.data.rows[i].name, value: response.data.rows[i].value})
          }
      ...
      saveCustomInfo() {
        ...
        try {
          ...
          const response = axios.post(upload_url + "save_custom_handler_react.php?id=" + id, qs.stringify({
      ...
      deleteImg() {
        ...
        try {
          ...
          const response = axios.post(upload_url + "api_sfc_com.php", qs.stringify({
      ...
      componentDidUpdate(prevProps,prevState) {
        const {sorting, currentPage, pageSize, searchValue ,rows, loading} = this.state
        if(sorting !== prevState.sorting
          || currentPage !== prevState.currentPage
          || searchValue !== prevState.searchValue
          || pageSize !== prevState.pageSize || loading){
          ...
          render() {
            ...
            <TextField id="search-name" value={this.state.searchName} label="Name" style={{
              width: '200px', 'margin-top':'9px'
            }} onChange={this.changeSearchName}/>&nbsp;
            <TextField id="search-desc" value={this.state.searchDesc} label="Description" style={{
              width: '200px', 'margin-top':'9px'
            }} onChange={this.changeSearchDesc}/>&nbsp;
            <FormControl className={classes.formControl} style={{width: '200px'}}>
            <InputLabel htmlFor="tag_name_sel_search" className={classes.bootstrapFormLabel} style={{'margin-top':'10px', 'margin-left':'0px'}}>Tag Name</InputLabel><br/>
            <Select
            ...
            <TextField id="search-tag-value" value={this.state.searchTagValue} label="Tag Value" style={{
              width: '200px', 'margin-top':'9px'
            ...
            {loading && <Loading />}
            <CustomInfoDialog title='Custom Info' open={this.state.customInfo} handleClose={this.handleClose('customInfo')}
            handleSubmit={this.handleSubmit}
            tags={this.state.tags} description={this.state.description} id={this.state.id}
             newTag={this.newTag} removeTag={this.removeTagHandler} saveCustomInfo={this.saveCustomInfo} valueChange={this.handleValueChange} nameChange={this.handleNameChange} changeDescription={this.changeDescriptionHandler}>
            </CustomInfoDialog>
            <Dialog open={this.state.deleting} onClose={this.handleClose('deleting')} aria-labelledby="form-dialog-title">
            <DialogTitle id="form-dialog-title">Delete this</DialogTitle>
            <DialogContent>
            Delete IMG id, {this.state.deleteId}?
            </DialogContent>
            <DialogActions>
